...
export default function Modification(props) {
  ...
  const onChargeDonne = (donne) => {
    setinfoReglment({reglement_id: donne.reglement_id,nom: donne.libelle,desc:donne.description})
  }
  ...
  const onSub = async () => { //Ajout de donnees vers Laravel
    ...
    await axios.put(props.url + 'updateReglementFact', infoReglment)
        .then(res => {
  ...
  return (
    <>
      ...
      <Button icon={PrimeIcons.PENCIL} className='p-buttom-sm p-1 mr-4 ' tooltipOptions={{position: 'top'}} style={stylebtnRec} tooltip='Modifier' onClick={() => { onClick('displayBasic2'); onChargeDonne(props.data);  }} />
      <div className='grid'>
