...
import ChoixPrescr from '../../Facture/PatientNonFact/ChoixPrescr';
import axios from 'axios';
import moment from 'moment/moment';

export default function StatClient(props) {

    const messagesParPage = 14;



    const [donne, setdonne] = useState({ code_presc: '', nom_presc: '', date_deb: '', date_fin: '' });
    const [verfChamp, setverfChamp] = useState({ nom_presc: false })
    ...
    const loadData = async (starts, ends) => {
        await axios.post(props.url + `getStatPrescripteur`, {
            date_deb: moment(donne.date_deb).format('DD/MM/YYYY'),
            date_fin: moment(donne.date_fin).format('DD/MM/YYYY'),
            code_presc: donne.code_presc,
            starts: starts,
    ...
    return (
        <>
            <Dialog header={'STAT PRESCRIPTEUR'} visible={displayBasic2} className="lg:col-3 md:col-3 col-8 p-0" onHide={() => onHide('displayBasic2')}  >
                    ...
                    <div className='flex flex-column justify-content-center'>
                        <div className='m-3'>
                            ...
                            <div className='m-0 flex flex-row align-items-center  '>
                                <InputText id="username2" style={{ backgroundColor: 'rgb(251 251 251)', height: '40px', borderRadius: '4px', width: '100%', fontWeight: 'bold' }} aria-describedby="username2-help" className={verfChamp.nom_presc ? "form-input-css-tamby p-invalid" : "form-input-css-tamby"} name='code_presc' value={donne.nom_presc} readOnly />
                                <ChoixPrescr url={props.url} infoFacture={donne} setverfChamp={setverfChamp} verfChamp={verfChamp} setinfoFacture={setdonne} typeclient={'a'} />
